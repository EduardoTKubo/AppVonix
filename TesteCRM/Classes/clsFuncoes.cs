...
namespace TesteCRM.Classes
{
    public static class clsFuncoes
    {
        ...
        public static DataTable[] CarregarListaUsuarios(string Cabecalho, string tabela, string rank, string ordem)
        {
            DataTable[] dtg = new DataTable[2];

            dtg[0] = clsBanco.Consulta("SELECT " + Cabecalho +
                    " FROM " + tabela + " WHERE ATIVO = 1 AND STATUS = 'OPERADOR' AND RANK <= " + rank +
                    " ORDER BY " + ordem + " ");
            dtg[1] = clsBanco.Consulta("SELECT " + Cabecalho +
                    " FROM " + tabela + " WHERE ATIVO = 1 AND STATUS <> 'OPERADOR' AND RANK <= " + rank +
                    " ORDER BY " + ordem + " ");
